...
export class PerformanceTest {
  ...
  private async generateBasicTestData(scale: Scale): Promise<TestData['basic']> {
  ...
  private async generateRelationalTestData(scale: Scale): Promise<TestData['relational']> {
    // For Postgres
    const usernameSet = new Set<string>();
    while (usernameSet.size < Math.floor(scale / 10)) {
      const username = `${faker.internet.username()}_${faker.string.nanoid(6)}`;
  ...
  async runTests(scale: Scale): Promise<TestResults> {
    ...
    const relationalResults = {
      postgresWithRelations: await this.runPostgresRelationsTests(scale, relationalData),
      mongoWithRelations: await this.runMongoRelationsTests(scale, relationalData)
    };
    await this.cleanup();  // Clean between test sets
  ...
  private async runPostgresBasicTests(scale: Scale, testData: TestData['basic']): Promise<any> {
    return {
      ...
      projectedRead: await this.measure("Postgres Basic Projected", async () => {
        await this.prisma.post.findMany({
          select: {
            title: true,
            created_at: true
  ...
  private async runPostgresRelationsTests(
    ...
    return {
      writes: await this.measure("Postgres Relations Writes", async () => {
        // Users are already created in generateRelationalTestData
        await this.prisma.post.createMany({ data: testData.posts });
  ...
  private async runMongoIndexedTests(
    ...
    // Similar to runMongoRelationsTests but uses indexes effectively
    // Will use the indexes we created in setupMongoIndexes()
    ...
    return {
      ...
      // ... Similar operations as runMongoRelationsTests, but with hints for indexes
      // Example for filteredRead:
  ...
  async runAggregationTests(): Promise<AggregationResults> {
    ...
    const results: AggregationResults = {
      postgres: {
        postsPerUser: await this.measure("Postgres Posts per User", async () => {
        ...
        avgLikesPerPost: await this.measure("Postgres Avg Likes per Post", async () => {
        ...
        mostActiveUsers: await this.measure("Postgres Most Active Users", async () => {
          await this.prisma.user.findMany({
            select: {
              username: true,
              posts: {
        ...
        mostLikedPosts: await this.measure("Postgres Most Liked Posts", async () => {
          await this.prisma.post.findMany({
            select: {
              title: true,
              user: {
        ...
        userEngagement: await this.measure("Postgres User Engagement", async () => {
          await this.prisma.$queryRaw`
            SELECT 
              u.username,
      ...
      mongo: {
        ...
        avgLikesPerPost: await this.measure("Mongo Avg Likes per Post", async () => {
          await db.collection("posts").aggregate([
            {
              $lookup: {
                from: "likes",
        ...
        mostActiveUsers: await this.measure("Mongo Most Active Users", async () => {
          await db.collection("users").aggregate([
            {
              $lookup: {
                from: "posts",
        ...
        mostLikedPosts: await this.measure("Mongo Most Liked Posts", async () => {
          await db.collection("posts").aggregate([
            {
              $lookup: {
                from: "likes",
        ...
        userEngagement: await this.measure("Mongo User Engagement", async () => {
          await db.collection("users").aggregate([
            // Posts created
            {
              $lookup: {
